...
public class AuthDbContext(DbContextOptions<AuthDbContext> options) : DbContext(options)
{
    ...
    protected override void OnModelCreating(ModelBuilder modelBuilder)
    {
        modelBuilder.Entity<User>(e =>
        {
            ...
            // Mapped as a string conversion (not Npgsql's native enum mapping) so the value
            // written is the exact C# member name ("CLIENT", "BUSINESS_OWNER", "SUPERADMIN"),
            // matching the labels the user_role Postgres enum is defined with in db/Spot.sql.
            // Npgsql's native enum mapping applies its own snake_case name translator, which
            // would instead write "client"/"business_owner" — invalid values for this type.
            e.Property(x => x.Role).HasColumnName("role").HasColumnType("user_role")
                .HasConversion<string>().HasDefaultValue(UserRole.CLIENT);
            ...
            // Enforces one row per email at the database level — the last word on duplicates,
            // regardless of any race between two concurrent registration requests.
